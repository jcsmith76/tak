# create Ansible inventory from AWS
...
# setup apt and python ahead of time if needed
ansible-playbook -i tf_inventory.txt provision.yml
sleep 100

# verify all the nodes are up and responding
ansible all --inventory-file=tf_inventory.txt -m ping -u ubuntu
sleep 100

# install the k8s master, join the workers to the master
